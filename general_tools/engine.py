...
A self-contained implementation of the Codex/Copilot-style agent loop:
    ...
    (guardrail envelope + audit) → repeat → final answer
...
from general_tools.IPP import IPP, ToolCallEvent, ToolContext
from LLMs.deepseek import DeepSeekProvider

logger = logging.getLogger("general_tools.engine")
...
class AgentEngine(IPP):
    ...
    def _tool_definitions(self, round_index: int = 1) -> list[dict]:
        # strict IPP: the definitions come from the tools node's F-file
        # catalog (list channel) — no api layer
    ...
    def _dispatch_tool(self, name: str, args: dict, ctx: "ToolContext") -> str:
        # strict IPP: route through the tools node's invoke channel (the
        # router R*_k resolves the target; both hops audited)
        from general_tools.construct import tools_node
