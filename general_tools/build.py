...
Output (project-scoped — see database/README.md):
  database/<active-project>/graph_data/knowledge_graph.json
  database/<active-project>/graph_data/vectors/index.json
No root-level graph_data/ exists; the active project is decided by
database/.active, exactly as the server does.
"""
...
logger = logging.getLogger("general_tools.build")
...
def export_backward_compatible(graph: KnowledgeGraph, out_root: Optional[Path] = None) -> Path:
    ...
    input.json/output.json into the active project's graph_data/export/.
    """
