...
class AgentRuntime:
    ...
    def stop(self, cancel_pending: bool = True) -> None:
        ...
        # JOIN the old thread so it cannot linger and race with a later
        # start() (two threads on the same queue would steal tasks).
        try:
            self._thread.join(timeout=5.0)
        except Exception:  # noqa: BLE001 — never block stop
            pass
    ...
    def enqueue(self, task: dict) -> None:
        ...
        # Ensure a worker is alive to process it. If the worker thread is
        # dead (previous stop / crash), restart it so the task is picked
        # up — otherwise it would sit in the queue forever.
        self._ensure_running()

    def _ensure_running(self) -> None:
        """Restart the worker if it is not alive or is stopping (idempotent)."""
        if self._thread.is_alive() and not self._stop:
            return
        # A live-but-stopping thread will exit on the `None` sentinel; a
        # dead thread cannot process anything. In both cases spawn a fresh
        # worker. Drain any stale sentinels first (never real tasks).
        try:
            while True:
                front = self._queue.get_nowait()
                if front is not None:
                    self._queue.put(front)
                    break
        except queue.Empty:
            pass
        self._stop = False
    ...
    def _run_loop(self) -> None:
        while not self._stop:
            try:
                ...
                self._set_status("error", f"queue error: {exc}")
                continue
            if task is None:
                break
            try:
                self._process(task)
            except Exception as exc:  # noqa: BLE001 — a task must never kill us
