"""
codex_growth.tools.IPP_object — the IPP Objects (Ω_k) of the growth tools node.

Handlers execute against the shared ToolRegistry, restricted to the
agent's tool set (GraphContext.bindings["tool_names"]). The four-phase
lifecycle (resolve → validate → prepare → invoke) runs inside H_invoke.
"""
...
def make_invoke_handler(bindings: dict):
    """H_invoke — one tool call through the four-phase lifecycle."""
...
def make_describe_handler(bindings: dict):
    """H_describe — one tool's JSON function definition."""
    tool_names = set(bindings.get("tool_names") or ToolRegistry.names())
