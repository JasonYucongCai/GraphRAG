...
LIVE_CHAT_STREAM_HANDLER = "IPP_Social.swarm.IPP_object:make_live_chat_stream_handler"
...
def finalize_agent_ipp(agent_folder: Path, force: bool = False) -> str:
    """Rewrite one agent's engine/ + tools/ ipp.json with its own identity.
...
- social_post: post to the global chat board. to='chat_board' broadcasts;
  to='agents' addresses every agent; to='<agent_id>' sends a direct
  inter-agent message (e.g. to='Codex_01_Alice').
- social_board / social_agents: read the board and discover the other
  agents (their ids and capacities) before addressing them.
