...
def make_chat_board_handler(bindings: dict):
    ...
    def handler(payload: dict, context: dict) -> dict:
        try:
            # strip None-valued keys before processing (O1 schema conformance)
            clean = {k: v for k, v in payload.items() if v is not None}
            op = clean.get("op")
            if op == "post":
                result = chat.post(
                    author_agent_id=clean.get("author_agent_id", ""),
                    text=clean.get("text", ""),
                    tags=clean.get("tags"),
                    to_agent_id=clean.get("to_agent_id", ""))
                return {"ok": True, **result}
            if op == "get":
                return {"ok": True, "messages":
                        [m.to_dict() for m in chat.get(clean.get("limit"))]}
            if op == "get_since":
                return {"ok": True, "messages":
                        [m.to_dict() for m in
                         chat.get_since(int(clean.get("after_id", 0) or 0))]}
            if op == "clear":
                return _clear(clean)
