...
Output (default): graph_data/cy3/knowledge_graph.json
                  graph_data/cy3/vectors/index.json
The default GraphRAG graph (graph_data/knowledge_graph.json, from tools.build)
is untouched — the two graphs live side by side.
...
from tools.build import export_backward_compatible  # noqa: F401 (re-export)
from tools.config import Config
from tools.encoder import EncoderLayer
from tools.graph import KnowledgeGraph, RELATION_VOCAB

logger = logging.getLogger("tools.build_cy3")
...
def build_cy3_graph(
    ...
    graph is persisted to ``out_dir`` (default ``graph_data/cy3/``) with the
